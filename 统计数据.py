...
while(today < datetime(2024,8,25)):
    songs_data_today =  pd.read_excel(f"数据/{(today + timedelta(days=1)).strftime('%Y%m%d')}与{today.strftime('%Y%m%d')}.xlsx", dtype={'pubdate':str},nrows=100)
    songs_data_new =  pd.read_excel(f"数据/新曲{(today + timedelta(days=1)).strftime('%Y%m%d')}与新曲{today.strftime('%Y%m%d')}.xlsx",nrows=10)
