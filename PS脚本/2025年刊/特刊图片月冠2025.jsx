...
function newImages() {
    var fileRef = new File(currentFolder + "特刊图片\\2025月冠样式.psd");
    if (fileRef.exists) {
        app.open(fileRef);
    } else {
    ...
    if (app.documents.length > 0) {
        ...
        if (data) {
            ...
            for (var i = 0; i < 88; i++) {
                ...
                layers.getByName("排名").textItem.contents = songData.month;
                ...
                fillDataExtend(songData, layers.getByName("播放").layers, ["view", "viewR"]);
                fillDataExtend(songData, layers.getByName("收藏").layers, ["favorite", "favoriteR"]);
                fillDataExtend(songData, layers.getByName("硬币").layers, ["coin", "coinR"]);
                fillDataExtend(songData, layers.getByName("点赞").layers, ["like", "likeR"]);

                $.writeln('完成第' + (i+1) + "张图片");
                savePic(doc, currentFolder + '月冠图片\\' + (i+1) + ".png");
            }
