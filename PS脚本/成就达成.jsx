...
function achievement() {
    var mode = judgeMode();
    var fileRef = new File(currentFolder + "其他图片\\成就达成.psd");
    app.open(fileRef);
    ...
    if (app.documents.length > 0) {
        ...
        var stats = readJSONFile(currentFolder + "成就.json");
        ...
        if (stats) {
            ...
            while (i<stats.length) {
                ...
                layers.getByName("数值").textItem.contents = songData.honor;
                setColor(layers.getByName("数值").textItem, colors[songData.honor]);
                ...
                if (pos == 4) {
                    savePic(doc, currentFolder + '其他图片\\成就达成' + (i+1)/5 + '.png')
                }
            ...
            savePic(doc, currentFolder + '其他图片\\成就达成' + ((i-pos)/5 + 1) + '.png')
        } else {
