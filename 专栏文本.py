...
同时收录作者投稿和搬运，包括画质提升、字幕版等。
            
一首曲目的同一版本仅收录其中一个，优先使用作者投稿。
            
如果没有作者投稿，则以本期得分最高的一个搬运作为数据。
...
如果被搬运一段时间后作者投稿，在10日内未上过主榜的可以进入新曲榜。
...
for i in range(9,-1,-1):
    ...
    song_box.append(new_tag("p", f"P主：{song_data['author']} {'作者投稿' if song_data['copyright'] == 1 else '搬运：' + song_data['uploader']} {song_data['type']}", class_ = "font-size-12"))
    ...
    song_box.append(new_tag("p", f"硬币：{song_data['coin']}{'（×' + str(song_data['fixA']) + '）' if song_data['fixA'] != 1.0 else ''}（×{song_data['coinR']}） {song_data['coin_rank']}位"))
    song_box.append(new_tag("p", f"点赞：{song_data['like']}（×{song_data['likeR']}） {song_data['like_rank']}位"))
    song_box.append(new_tag("p", f"得分：（×{song_data['fixB'] * song_data['fixC']:.2f}=）{song_data['point']}"))
