...
def statistics_today():
    ...
    with open(f"月刊/新版统计/{today.strftime('%Y-%m')}.json",'w',encoding='utf-8') as file:
...
for today in  datetime(2024,8,31), datetime(2024,9,30):
    print(today)
    songs_data_today =  pd.read_excel(f"月刊/数据/{today.strftime('%Y-%m')}.xlsx", dtype={'author':str, 'pubdate':str})
    songs_data_new =  pd.read_excel(f"月刊/数据/新曲{today.strftime('%Y-%m')}.xlsx",nrows=10)
